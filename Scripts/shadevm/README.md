...
shadeVM is a streamlined development tool that automatically sets up shade in a virtual machine for testing different branches and commits.

- [shadeVM - Simplified VM Tool for shade Contributors](#shadevm---simplified-vm-tool-for-shade-contributors)
...
- **Zero Configuration**: Automatically downloads Arch Linux base image and sets up shade
- **Branch Testing**: Easily test any shade branch or commit hash
...
When you run a new branch/commit for the first time, shadevm will:
...
4. **shade Installation**: You'll need to:
   - Login as `arch` / `arch`
   - Run the provided curl command to download and execute the setup script
   - Wait for shade installation to complete
...
shadevm feature-branch
shadevm abc123def

# Run with persistence (changes will be saved)
shadevm --persist
shadevm --persist dev-branch

# Run headless (SSH-only, no display window)
shadevm --ssh-only
shadevm --ssh-only --persist dev-branch

# Clone an existing snapshot as a new branch for testing
shadevm --clone master my-test-branch

# Clone and run headless
shadevm --ssh-only --clone master my-test-branch
...
shadevm --install-deps
```

### Checkpoint Workflow (Save/Restore)

shadeVM supports saving and restoring the **full live VM state** (disk + RAM + device state) using QEMU's internal snapshot mechanism. This is perfect for testing where you need to return to exact checkpoints.

```bash
# Terminal 1: Start a VM (works with or without --persist)
shadevm my-branch
# or: shadevm --persist my-branch  (for permanent checkpoints)

# Terminal 2: Save a checkpoint while the VM is running
shadevm --snapshot-as before-test my-branch
...
shadevm --checkpoint before-test my-branch
# (Restores exact disk + RAM state - back to square one)

# List checkpoints in a snapshot
shadevm --list-snapshots my-branch
```

**How it works:**

- Every running VM exposes a QMP Unix socket at `~/.cache/shadevm/qmp-<branch>.sock`
...
VM_MEMORY=8G VM_CPUS=4 shadevm

# Set extra QEMU arguments
VM_EXTRA_ARGS="-display vnc=:1" shadevm

# Override QEMU command entirely, provided $VM_DISK will be substituted with the actual disk image
VM_QEMU_OVERRIDE="qemu-system-x86_64 -m 4G -smp 2 -enable-kvm -drive file=\$VM_DISK,format=qcow2,if=virtio -device virtio-vga -display gtk" shadevm
```
...
- **Cache Directory**: Uses XDG Base Directory specification (`$XDG_CACHE_HOME/shadevm/`)
- **Snapshots**: Stored in `$XDG_CACHE_HOME/shadevm/snapshots/` (typically `~/.cache/shadevm/snapshots/`)
- **Base Image**: Cached in `$XDG_CACHE_HOME/shadevm/archbase.qcow2` (typically `~/.cache/shadevm/archbase.qcow2`)
...
```bash
shadevm --clean  # Remove all cached data from $XDG_CACHE_HOME/shadevm/
```
...
shade uses Hyprland, which has specific requirements for VM environments. Hyprland VM support is limited - see [Hyprland - Running in a VM](https://wiki.hypr.land/Getting-Started/Installation/#running-in-a-vm) for official guidance.

> [!NOTE]
> I'm trying here to make shade easier to work with VM's. If you have any suggestions based on your hardware and experience, or find this documentation inaccurate, please let me know.
...
VM_EXTRA_ARGS="-device virtio-vga -display gtk,gl=off" shadevm
```
...
VM_EXTRA_ARGS="-device VGA -display gtk,gl=off" shadevm
```
...
VM_QEMU_OVERRIDE="qemu-system-x86_64 -m 4G -smp 2 -enable-kvm -cpu host -machine q35 -device intel-iommu -drive file=\$VM_DISK,format=qcow2,if=virtio -device virtio-vga-gl -display gtk,gl=on,grab-on-hover=on -usb -device usb-tablet -device ich9-intel-hda -device hda-output -vga none" shadevm
