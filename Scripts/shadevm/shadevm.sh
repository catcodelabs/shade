...
function print_usage() {
    echo "shadeVM - Simplified VM tool for shade contributors"
    echo "Supports: Arch Linux, NixOS"
    echo ""
    echo "Usage: shadevm [OPTIONS] [BRANCH/COMMIT]"
    echo ""
    echo "Arguments:"
    ...
    echo ""
    echo "Examples:"
    echo "  shadevm                               # Run master branch"
    echo "  shadevm --persist                     # Run master branch (persistent)"
    echo "  shadevm --ssh-only                    # Run master headless (SSH-only)"
    echo "  shadevm --persist dev                 # Run dev branch with persistence"
    echo "  shadevm --mount ~/shade                # Share local shade folder live into the VM"
    echo "  shadevm --mount ~/shade --ssh-only     # Headless with live folder sharing"
    echo "  shadevm --ssh-only --persist          # Run master headless with persistence"
    echo "  shadevm --snapshot-from master my-test # Clone master snapshot as my-test"
    echo ""
...
function install_all_arch_dependencies() {
    ...
    echo "📦 Installing all shadeVM dependencies..."
...
function check_deps_only() {
    local os
    os=$(detect_os)
    echo "🔍 Checking shadeVM dependencies..."
    echo "   Detected OS: $os"
...
function create_shade_snapshot() {
    local ref="${1:-master}"
    local ssh_only="${2:-false}"
    local mount_path="${3:-}"
    local snapshot_name
    snapshot_name=$(get_snapshot_name "$ref")
    local snapshot_path="$SNAPSHOTS_DIR/shade-$snapshot_name.qcow2"
    local qemu_cmd
    qemu_cmd=$(get_qemu_command)
    local python_cmd
    ...
    echo "🔨 Creating shade snapshot for '$ref'..."

    # If --mount is provided, symlink the local shade repo so setup.sh can use it
    local local_repo=""
    if [ -n "$mount_path" ] && [ -d "$mount_path" ]; then
        local_repo="$mount_path"
        echo "📁 Will use local shade repo from mount: $local_repo"
    fi
...
echo "🚀 Setting up shade environment for branch/commit: $ref"

# Set root password for convenience
echo "🔐 Setting root password..."
echo -e "shadevm\nshadevm" | sudo passwd root
...
# Clone or update shade repository
echo "📥 Setting up shade repository..."
cd /home/arch
if [ -d "shade" ]; then
    echo "   shade directory exists, updating..."
    cd shade
    git fetch origin
    git reset --hard HEAD  # Reset any local changes
else
    echo "   Cloning shade repository..."
    git clone "$SHADE_REPO" shade
    cd shade
fi
...
[Unit]
Description=shade host 9p mount
[Mount]
What=host_share
Where=/mnt/host
Type=9p
...
if mountpoint -q /mnt/host 2>/dev/null; then
    echo "🔗 Host mount detected at /mnt/host — using local shade repo!"
    echo "   This gives you live updates as you edit files on the host."
    rm -rf /home/arch/shade
    ln -sf /mnt/host /home/arch/shade
fi

# Check if shade is already installed
if [ -f "/home/arch/.config/hypr/hyprland.conf" ] && [ -f "/home/arch/.config/shade/shade.conf" ]; then
    echo "⚠️  shade appears to already be installed."
    echo "   Configuration files found. Skipping installation."
    echo "   If you want to reinstall, remove ~/.config/hypr and ~/.config/shade first."
else
    echo "🚀 Starting shade installation..."
    cd /home/arch/shade/Scripts
    ./install.sh
    echo "✅ shade installation complete!"
fi
...
SETUP_EOF
    ...
    echo ""
    echo "🖥️  Starting VM for shade installation..."
    ...
    cd "$CACHE_DIR"
    # TODO: feat(shadevm) migrate from the python http server to a pure ssh solution, no setup script needed
    ...
    echo "✅ Snapshot created: shade-$snapshot_name"
    echo "🚀 You can now run: shadevm $ref"
}
...
function duplicate_snapshot() {
    ...
    if [ -z "$source_ref" ] || [ -z "$target_ref" ]; then
        echo "❌ Usage: shadevm --snapshot-from <source> <target>"
        return 1
    fi
    ...
    local source_path="$SNAPSHOTS_DIR/shade-$source_snap.qcow2"
    local target_path="$SNAPSHOTS_DIR/shade-$target_snap.qcow2"

    if [ ! -f "$source_path" ]; then
        echo "❌ Source snapshot for '$source_ref' not found."
        echo "   Run 'shadevm --list' to see available snapshots."
        return 1
    fi
    ...
    echo "✅ Snapshot cloned: shade-$target_snap"
    echo "   Run: shadevm $target_ref"
    echo "   Or:  shadevm --ssh-only $target_ref"
    echo "   Or:  shadevm --persist $target_ref"
}
...
function run_vm() {
    ...
    if [ ! -f "$snapshot_path" ]; then
        ...
        create_shade_snapshot "$ref" "$ssh_only" "$mount_path"
    fi
    ...
    echo "🚀 Starting shade VM (branch/commit: $ref)..."
...
function list_snapshots() {
    echo "📸 Available shade snapshots:"
    if [ -d "$SNAPSHOTS_DIR" ]; then
        local count=0
        while IFS= read -r snap; do
            echo "   - $snap"
            count=$((count + 1))
        done < <(find "$SNAPSHOTS_DIR" -name "shade-*.qcow2" -exec basename {} \; | \
            sed 's/^shade-//' | sed 's/\.qcow2$//' | sort)
        if [ "$count" -eq 0 ]; then
...
function snapshot_as() {
    ...
    qemu-img create -f qcow2 -F qcow2 -b "$source_path" "$target_path"
    echo "✅ Snapshot saved as: shade-$target_snap"
    echo "   Run: shadevm $target_name"
    echo "   Or:  shadevm --persist $target_name"
    echo "   Or:  shadevm --ssh-only $target_name"
}

function clean_cache() {
    echo "🧹 Cleaning shadeVM cache..."
    rm -rf "$CACHE_DIR"
    echo "✅ Cache cleaned"
}
