...
Hyprland 0.56 warns against the old `hyprlang` config format, so shade moved its
entire Hyprland configuration to Lua. If you installed shade before that release,
this page covers what changed, what breaks, and what to do about it.

> [!NOTE]
> For the most comprehensive guide on migration, it is highly recommended to check the shade wiki first: [Lua for the User & Migration](https://shadeproject.pages.dev/en/help/lua/)

## What changed

| Before | Now |
| :--- | :--- |
| `~/.config/hypr/hyprland.conf` and the files it sourced | `~/.local/share/hypr/shade.lua` |
...
| `~/.config/hypr/windowrules.conf`, `monitors.conf`, `nvidia.conf` | your own `hyprland.lua`, or shade's `lua/` modules |
| `~/.config/hypr/animations.conf`, `workflows.conf` | `shade-shell animations --select`, `shade-shell workflows --select` |

Hyprland is started against `~/.local/share/hypr/shade.lua`. That file loads
shade's modules, then shade's own keybinds, then yours, and last the workflow you
...
```bash
cd ~/shade/Scripts   # wherever you cloned it
git pull
./install.sh -r
```
...
| :--- | :--- |
| A bare session or a black screen with just a cursor when starting from a TTY | You are pointing Hyprland at the wrong file. Use `Hyprland -c ${XDG_DATA_HOME:-$HOME/.local/share}/hypr/shade.lua`, or start through `start-hyprland`. `~/.config/hypr/hyprland.lua` is loaded *by* shade, it is not the entry point |
| The session refuses to start, `No valid shade configuration found` | The Lua runtime is missing. Install `lua` and `luarocks` |
| A red overlay: `Hyprland does not detect colors! Run: shade-shell reload` | Theme colours were never generated for the Lua state. Run `shade-shell reload` |
...
| <kbd>SUPER</kbd> + <kbd>/</kbd> says the hint failed to initialise | A stale hint cache. Run `shade-shell keybinds_hint --reload` |
| An unexpected yellow tint, or workspace 1 on the wrong monitor | Old state carried over. Check `shade-shell shaders --select` and your monitor settings in `~/.config/hypr/hyprland.lua` |
...
~/.local/share/shade/{hyprland,keybindings}.conf
```
...
- `~/.config/systemd/user/shade-config.service` and `shade-ipc.service` no longer
  exist upstream. If you enabled them, they will keep failing:
  `systemctl --user disable --now shade-config.service shade-ipc.service`
- `~/.config/fish/completions/shade-shell.fish` and
  `~/.config/zsh/completions/shade-shell.zsh` moved into shade's library
...
in the [Hyprland wiki](https://wiki.hypr.land/Configuring/). The shade side —
`shade.sh`, the command map and the bind flags — is in
