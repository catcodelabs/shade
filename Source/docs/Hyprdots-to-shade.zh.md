...
接下来我们要谈谈如何在不改变用户配置的情况下更新 shade 相关的配置文件。
我们不再需要 “userprefs” 文件。相对的，我们可以引用(source) shade 的 hyprland.conf 并允许\$USER 直接在配置中更改。通过这个方式，您不会破坏 shade，shade 也不会破坏您的配置文件。
