...
handle_on_lua() {
    ui_state="${XDG_STATE_HOME}/shade/lua_state/ui.lua"
    wallbash_mode="theme"
    ...
    local _shade_theme _gtk_theme _icon_theme _color_scheme
    ...
    lua_quote_to _shade_theme "${SHADE_THEME}"
    ...
    # ? UI config state
    cat <<ON_WALLBASH >"${ui_state}" && print_log -sec "theme" -stat "Updating shade Lua UI state for wallbash mode"
-- Auto-generated by shade // Read-only
local ui_state = {
    ui = {
        shade_theme = "$_shade_theme",
        -- gtk
