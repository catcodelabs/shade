...
local function build_option_context_with_detection()
    ...
    ctx:set_description(
        [[
shade-shell: open files, MIME types, or keywords with desktop applications.

Examples:
  shade-shell open example.pdf
  shade-shell open text-editor
  shade-shell open example.pdf --with evince.desktop --set-default
]]
    )
    ...
    ctx:add_main_entries(entries, "shade-shell")
...
local function print_usage(prog)
    prog = prog or arg[0] or "shade-shell"
