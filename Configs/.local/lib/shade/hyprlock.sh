#! /bin/bash
if ! source "$(which shade-shell)"; then
    echo "Error: shade-shell not found."
    echo "Is shade installed?"
    exit 1
fi
...
cacheDir="${SHADE_CACHE_HOME:-"$XDG_CACHE_HOME/shade"}"
WALLPAPER="$cacheDir/wall.set"
HYPRLOCK_SCOPE_NAME="shade-${XDG_SESSION_DESKTOP:-unknown}-lockscreen.scope"
...
fn_background() {
    ...
    if [ "$is_video" -eq 1 ]; then
        ...
        mkdir -p "$SHADE_CACHE_HOME/wallpapers/thumbnails"
        cached_thumb="$SHADE_CACHE_HOME/wallpapers/$(${hashMech:-sha1sum} "$WP" | cut -d' ' -f1).png"
...
fn_profile() {
    ...
    if [ -f "$HOME/.face.icon" ]; then
        ...
        cp "$XDG_DATA_HOME/icons/Wallbash-Icon/shade.png" "$profilePath.png"
    fi
    return 0
}
fn_mpris() {
    ...
    if [[ $player_status == "Playing" || $player_status == "Paused" ]]; then
        ...
        if [ -f "$HOME/.face.icon" ]; then
            ...
            if ! cmp -s "$XDG_DATA_HOME/icons/Wallbash-Icon/shade.png" "$THUMB.png"; then
                cp "$XDG_DATA_HOME/icons/Wallbash-Icon/shade.png" "$THUMB.png"
                reload_hyprlock
            fi
        fi
        exit 1
...
fn_cava() {
    ...
    config_file="$XDG_RUNTIME_DIR/shade/cava.hyprlock"
    if [ "$(pgrep -c -f "cava -p $config_file")" -eq 0 ]; then
        trap 'rm -f ${tempFile}' EXIT
        "${LIB_DIR}/shade/cava.py" hyprlock >$tempFile 2>&1
    fi
}
fn_art() {
...
find_filepath() {
    ...
    local search_dirs=(
        ...
        "${XDG_CONFIG_HOME:-$HOME/.config}/shade/hyprlock"
        "$HYPRLOCK_CONF_DIR")
...
$layout_items"
    selected_layout=$(awk -F/ '{print $NF}' <<<"$layout_items" | rofi -dmenu -i -select "$HYPRLOCK_LAYOUT" \
        ...
        -on-selection-changed 'shade-shell hyprlock.sh --test-preview  "{entry}"' \
    ...
    "$LIB_DIR/shade/font.sh" resolve "$hyprlock_conf_path"
    fn_profile
...
layout_test() {
    ...
    sleep 2
    local temp_path="$XDG_RUNTIME_DIR/shade/hyprlock-test.conf"
...
rofi_test_preview() {
    ...
    local unit_name="shade-${XDG_SESSION_DESKTOP:-unknown}-lockscreen-preview.scope"
...
generate_conf() {
    ...
    local shade_hyprlock_conf=${SHARE_DIR:-$XDG_DATA_HOME}/hypr/hyprlock.conf
...
#*│ \$XDG_CONFIG_HOME/hypr/shade.conf,                                          │
...
#*┌──────────────────────────────────────────────────────────────────────────┐
#*│    All boilerplate configurations are handled by shade                  │
#*└──────────────────────────────────────────────────────────────────────────┘

source = $shade_hyprlock_conf
...
#┌────────────────────────────────────────────────────────────────────────────┐
#│ shade also provides custom variables to extend hyprlock's functionality.  │
#│                                                                            │
#│   \$BACKGROUND_PATH                                                        │
...
#│   - if available, otherwise, it will show the shade logo.                   │
#│                                                                            │
#│   \$PROFILE_IMAGE                                                          │
...
#│   - shade will run 'font.sh resolve' to install the font for you.           │
...
fn_reload() {
    local layout="${HYPRLOCK_LAYOUT:-shade}"
...
ensure_lockscreen_bg_exist() {
    if [ ! -f "$SHADE_CACHE_HOME/wallpapers/hyprlock.png" ]; then
        print_log -sec "hyprlock" -stat "setting" " $SHADE_CACHE_HOME/wallpapers/hyprlock.png"
        "${LIB_DIR}/shade/wallpaper.sh" -s "$(readlink "$SHADE_THEME_DIR/wall.set")" --backend hyprlock
    fi
}

argparse_init "$@"
argparse_header "Hyprlock Manager"
argparse_program "shade-shell hyprlock"
argparse_footer "Use 'shade-shell hyprlock --help' for more information."
...
argparse "--reload,-r" "" "Regenerates hyprlock.conf using HYPRLOCK_LAYOUT (defaults to shade)"
...
*)
    ensure_lockscreen_bg_exist
    check_and_sanitize_process
    "${LIB_DIR}/shade/app.sh" -u "$HYPRLOCK_SCOPE_NAME" -t scope -- hyprlock
    exit 0
    ;;
esac
