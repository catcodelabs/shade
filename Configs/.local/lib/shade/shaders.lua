...
local COMPILED = xdg.state .. "/shade/compiled.cache.glsl"
...
-- referencing "$XDG_CACHE_HOME" still resolves outside of a shade session.
local ENV_FALLBACK = {
...
local function compile_shader(item)
    ensure_dir(xdg.state .. "/shade/shaders")
...
if _is_main then
    ...
    local parser = argparse("shade-shell shaders", "shade Shader Selector")
