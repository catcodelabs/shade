...
if ! source "$(which shade-shell)"; then
    echo "[$0] :: Error: shade-shell not found."
    echo "[$0] :: Is shade installed?"
    exit 1
fi
sunsetConf="${XDG_STATE_HOME:-$HOME/.local/state}/shade/hyprsunset"
default_temp=6000
default_gamma=100
temp_step=500
gamma_step=5
...
send_notification() {
    ...
    if [ -n "$message" ]; then
        notify-send -a "shade Notify" -r 19 -t 800 -i redshift "$message" "$title"
    else
        notify-send -a "shade Notify" -r 19 -t 800 -i redshift "$title"
    fi
}
