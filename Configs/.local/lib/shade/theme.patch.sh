...
WALLBASH_DIRS=(
    "${XDG_CONFIG_HOME:-$HOME/.config}/shade/wallbash"
    "${XDG_DATA_HOME:-$HOME/.local/share}/shade/wallbash"
...
if [ -d "$2" ]; then
    ...
    git_theme=${git_path#*/}
    branch_dir=${branch//\//_}
    cache_dir="${XDG_CACHE_HOME:-"$HOME/.cache"}/shade"
...
FAV_THEME_DIR="$THEME_DIR/Configs/.config/shade/themes/$THEME_NAME"
[ ! -d "$FAV_THEME_DIR" ] && print_log -r "[ERROR] " "'$FAV_THEME_DIR'" -y " Do not Exist" && exit 1
config=$(find -H "${WALLBASH_DIRS[@]}" -type f -path "*/theme*" -name "*.dcol" 2>/dev/null | awk '!seen[substr($0, match($0, /[^/]+$/))]++' | awk -v favTheme="$THEME_NAME" -F 'theme/' '{gsub(/\.dcol$/, ".theme"); print ".config/shade/themes/" favTheme "/" $2}')
restore_list=""
...
if [ -f "$FAV_THEME_DIR/theme.dcol" ]; then
    ...
    restore_list+="Y|Y|\${HOME}/.config/shade/themes/$THEME_NAME|theme.dcol|hyprland\n"
fi
...
theme_wallpapers="$confDir/shade/themes/$THEME_NAME/wallpapers"
[ ! -d "$theme_wallpapers" ] && mkdir -p "$theme_wallpapers"
while IFS= read -r walls; do
    cp -f "$walls" "$theme_wallpapers"
done <<<"$wallpapers"
theme_logos="$confDir/shade/themes/$THEME_NAME/logo"
if [ -n "$logos" ]; then
