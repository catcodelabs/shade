...
STATE_FILE = Path(os.path.join(str(xdg_state_home()), "shade", "staterc"))
SHADE_CONFIG = Path(os.path.join(str(xdg_state_home()), "shade", "config"))
UNIT_NAME = f"shade-{os.environ.get('XDG_SESSION_DESKTOP', 'unknown')}-bar.service"
...
def get_config_value(key, default=None):
    ...
    if SHADE_CONFIG.exists():
        with open(SHADE_CONFIG, "r") as file:
            for line in file:
...
def write_style_file(style_filepath, source_filepath):
    ...
    wallbash_gtk_css_file = os.path.join(str(xdg_cache_home()), "shade", "wallbash", "gtk.css")
...
def get_value_from_hypr_theme(variable_name):
    ...
    if STATE_FILE.exists():
        try:
            with open(STATE_FILE, "r") as file:
                for line in file:
                    if line.startswith("SHADE_THEME="):
    ...
    hypr_theme_path = os.path.join(str(xdg_config_home()), "shade", "themes", theme_name, "hypr.theme")
...
def update_style(style_path):
    ...
    if not os.path.exists(theme_style_filepath):
        logger.error(
            f"Missing '{theme_style_filepath}', Please run 'shade-shell reload' to generate it."
        )
...
def main():
    ...
    source_env_file(os.path.join(str(xdg_runtime_dir()), "shade", "environment"))
    source_env_file(os.path.join(str(xdg_state_home()), "shade", "config"))
