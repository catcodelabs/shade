#!/usr/bin/env bash
# Bash completion for shade-shell

_shade_shell_completion() {
...
" 2>/dev/null)
    ...
    if [[ $cword -eq 1 ]]; then
        COMPREPLY=($(compgen -W "$(__shade_shell_run --list-all)" -- "$cur"))
        return 0
    fi
    ...
    if [[ $cword -eq 2 ]]; then
        case "$prev" in
            wallbash)
                COMPREPLY=($(compgen -W "$(__shade_shell_run --list-script)" -- "$cur"))
                return 0
                ;;
            ...
            *)
                local script_cmds
                script_cmds=$(__shade_shell_run --list-commands "$prev")
    ...
    if [[ $cword -ge 3 ]]; then
        ...
        local opts
        opts=$(__shade_shell_run --list-options "$cmd" "$subcmd")
        local flags
        flags=$(__shade_shell_run --list-flags "$cmd" "$subcmd")
...
complete -F _shade_shell_completion shade-shell
