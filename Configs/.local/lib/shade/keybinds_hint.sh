...
kb_cache="$XDG_RUNTIME_DIR/shade/keybinds_hint.rofi"

if [[ $1 == "--reload" ]]; then
    "${LIB_DIR}/shade/keybinds/hint-hyprland.py" --format rofi >"$kb_cache" && echo "Key Bind cache updated"
    exit 0
fi

[ -f "$kb_cache" ] && {
    trap '${LIB_DIR}/shade/keybinds/hint-hyprland.py --format rofi > "$kb_cache" && echo "Key Bind cache updated" ' EXIT
}
output="$(if
    ! cat "$kb_cache" 2>/dev/null
then
    "${LIB_DIR}/shade/keybinds/hint-hyprland.py" --format rofi | tee "$kb_cache"
fi)"
wait
if [ -z "$output" ]; then
