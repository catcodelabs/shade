...
notify-send "DEPRECATION NOTICE: swww backend is deprecated, please switch to awww or other supported backends. See 'shade-shell wallpaper --help' for more info."

selected_wall="${1:-"$$SHADE_CACHE_HOME/wall.set"}"
lockFile="$XDG_RUNTIME_DIR/shade/$(basename "$0").lock"
if [ -e "$lockFile" ]; then
    cat << EOF
