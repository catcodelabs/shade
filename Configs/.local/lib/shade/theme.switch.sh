...
[ -z "$SHADE_THEME" ] && echo "ERROR: unable to detect theme" && exit 1
get_themes
confDir="${XDG_CONFIG_HOME:-$HOME/.config}"
Theme_Change() {
    local x_switch=$1
    for i in "${!thmList[@]}"; do
        if [ "${thmList[i]}" == "$SHADE_THEME" ]; then
            if [ "$x_switch" == 'n' ]; then
...
[[ ! " ${thmList[*]} " =~ " $themeSet " ]] && themeSet="$SHADE_THEME"
set_conf "SHADE_THEME" "$themeSet"
print_log -sec "theme" -stat "apply" "$themeSet"
export reload_flag=1
source "$LIB_DIR/shade/globalcontrol.sh"
source "$SHARE_DIR/shade/env-theme"
if [[ -r $HYPRLAND_CONFIG ]]; then
    if [[ -n $HYPRLAND_INSTANCE_SIGNATURE ]]; then
        hyprctl eval 'hl.config({misc = {disable_autoreload = true}})'
    fi
    if [[ -r "$SHADE_THEME_DIR/hypr.theme" ]]; then
        print_log -sec "theme" -stat "dump" "hypr.theme to lua"
        theme_state="$XDG_STATE_HOME/shade/lua_state/hypr_theme.lua"
        theme_buffer="$(mktemp)"
        if hyq --dump "$SHADE_THEME_DIR/hypr.theme" --schema "$XDG_DATA_HOME/hypr/schema/hyprland-lua.json" --export lua >"$theme_buffer" &&
            [ -s "$theme_buffer" ] &&
    ...
    load_hypr_variables "$SHADE_THEME_DIR/hypr.theme"                               # ? loads the theme vars
    load_hypr_variables "${XDG_STATE_HOME:-$HOME/.local/state}/shade/hyprland.conf" # ? loads the parsable user config vars, should override theme vars
fi
show_theme_status
...
if [[ -d "$SHADE_CACHE_HOME/wallpapers/" ]]; then
    find -H "$SHADE_CACHE_HOME/wallpapers" -name "*.png" -exec sh -c '
        for file; do
            base=$(basename "$file" .png)
            if pkg_installed ${base}; then
                "${LIB_DIR}/shade/wallpaper.sh" --link --backend "${base}"
            fi
        done
    ' sh {} + &
    _link_pid=$!
...
theme_wallpaper="$(readlink "$SHADE_THEME_DIR/wall.set")"
if [ -z "$theme_wallpaper" ] || [ ! -e "$theme_wallpaper" ]; then
    if [ -d "$SHADE_THEME_DIR/wallpapers" ]; then
        theme_wallpaper="$(find -H "$SHADE_THEME_DIR/wallpapers" -type f -print -quit)"
    else
        print_log -sec "theme" -warn "wallpaper" "$SHADE_THEME carries no wallpapers directory"
        theme_wallpaper=""
    fi
    [ -n "$theme_wallpaper" ] && ln -fs "$theme_wallpaper" "$SHADE_THEME_DIR/wall.set"
fi
if [ -z "$theme_wallpaper" ]; then
    print_log -sec "theme" -crit "error" "no wallpaper available for $SHADE_THEME"
    exit 1
fi
wallpaper_failure_is_fatal() {
    case "$1" in
    "${SHADE_STATUS_CACHE_FAILED:-3}" | "${SHADE_STATUS_COLOURS_FAILED:-4}") return 0 ;;
    esac
    return 1
}
...
wallpaper_status=0
if [ "$quiet" = true ]; then
    wallpaper_output="$("$LIB_DIR/shade/wallpaper.sh" -s "$theme_wallpaper" --global 2>&1)" || wallpaper_status=$?
else
    "$LIB_DIR/shade/wallpaper.sh" -s "$theme_wallpaper" --global || wallpaper_status=$?
fi
if [ "$wallpaper_status" -ne 0 ]; then
    [ -n "${wallpaper_output:-}" ] && printf '%s\n' "$wallpaper_output" >&2
    if wallpaper_failure_is_fatal "$wallpaper_status"; then
        print_log -sec "theme" -crit "error" "generating the colour state for $SHADE_THEME failed with $wallpaper_status, the state on disk is the one the previous theme left"
        exit 1
    fi
...
if ! wallbash_state_is_complete; then
    print_log -sec "theme" -stat "colours" "generating the colour state for $SHADE_THEME"
    if ! "$LIB_DIR/shade/color.set.sh" "$theme_wallpaper"; then
        print_log -sec "theme" -crit "error" "could not generate the colour state from $theme_wallpaper"
        exit 1
    fi
fi
if ! wallbash_state_is_complete; then
    print_log -sec "theme" -crit "error" "colour state is still incomplete for $SHADE_THEME"
    exit 1
fi
