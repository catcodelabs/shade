...
local config = {
    ConfigFile = xdg_config_home .. "/shade/config.toml",
    EnvFile = xdg_state_home .. "/shade/config",
    HyprFile = xdg_state_home .. "/shade/hyprland.conf",
    HyprEnvFile = xdg_state_home .. "/shade/hypr.conf",
    NoDaemon = false,
    NoExport = false,
    Verbose = false,
    Debug = false
...
local shadeConfigNotificationID = 19
local isInitialStartup = true

-- CLI parsing
local function parse_args(argv)
    local parser = argparse("shade-shell config", "Parse shade config.toml and generate shade/Hyprland output files.")
...
local function load_toml_file(path)
    ...
    if not ok then
        ...
        if not config.NoDaemon then
            ...
            notify.send(
                "shade Config: TOML Syntax Error",
                msg,
                {urgency = "normal", replace_id = shadeConfigNotificationID}
            )
        end
...
local function parse_config_files(tomlFile, envFile, hyprFile, hyprEnvFile, exportMode)
    ...
    if success and not config.NoDaemon and not isInitialStartup then
        sendSuccessNotification("shade Config", "Configuration reloaded successfully")
    end
