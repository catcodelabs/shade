...
-- Bootstrap shade luautils paths
...
local runtime_dir = conf.XDG_RUNTIME_DIR .. '/shade'

local function show_config()
    io.write('\nModify ' .. conf.XDG_CONFIG_HOME .. '/shade/config.toml to set options.\n')
...
local function handle_update()
    ...
    if not conf.dock then
        ...
        elseif status == 'Full' then
            ...
            local fname = '/tmp/shade.battery.notify.status.fallback.' ..
