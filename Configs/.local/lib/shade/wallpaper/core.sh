...
Wall_Json() {
    setIndex=0
    [ ! -d "$SHADE_THEME_DIR" ] && echo "ERROR: \"$SHADE_THEME_DIR\" does not exist" && exit 0
    if [ -d "$SHADE_THEME_DIR/wallpapers" ]; then
        wallPathArray=("$SHADE_THEME_DIR/wallpapers")
    else
        wallPathArray=("$SHADE_THEME_DIR")
    fi
    wallPathArray+=("${WALLPAPER_CUSTOM_PATHS[@]}")
    [ -d "${XDG_PICTURES_DIR%/}/Wallpapers/${SHADE_THEME}" ] && wallPathArray+=("${XDG_PICTURES_DIR%/}/Wallpapers/${SHADE_THEME}")
    get_hashmap "${wallPathArray[@]}"
    wallListJson=$(printf '%s\n' "${wallList[@]}" | jq -R . | jq -s .)
    wallHashJson=$(printf '%s\n' "${wallHash[@]}" | jq -R . | jq -s .)
    jq -n --argjson wallList "$wallListJson" --argjson wallHash "$wallHashJson" --arg cacheHome "${SHADE_CACHE_HOME:-$HOME/.cache/shade}" '
...
#   SHADE_STATUS_CACHE_FAILED, SHADE_STATUS_COLOURS_FAILED
# Returns:
#   0 on success, SHADE_STATUS_CACHE_FAILED when the thumbnail cache fails,
#   SHADE_STATUS_COLOURS_FAILED when the colour pass fails
##
Wall_Cache() {
    ...
    if [ "$set_as_global" == "true" ]; then
        ...
        local cache_output
        if ! cache_output=$("$LIB_DIR/shade/wallpaper/cache.sh" commence -w "${wallList[setIndex]}" 2>&1); then
            print_log -sec "wallpaper" -err "cache" "could not cache ${wallList[setIndex]}"
            printf '%s\n' "$cache_output" >&2
            return "${SHADE_STATUS_CACHE_FAILED:-3}"
        fi
        if ! "$LIB_DIR/shade/color.set.sh" "${wallList[setIndex]}"; then
            print_log -sec "wallpaper" -err "colors" "could not generate colours from ${wallList[setIndex]}"
            return "${SHADE_STATUS_COLOURS_FAILED:-4}"
        fi
