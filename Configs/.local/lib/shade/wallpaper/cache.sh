...
wallpaper_cache_bootstrap() {
    if [[ ${SHADE_SHELL_INIT:-0} -ne 1 ]]; then
...
wallpaper_cache_init() {
    [ -d "$SHADE_THEME_DIR" ] && cacheIn="$SHADE_THEME_DIR" || {
        echo "Error: SHADE_THEME_DIR not found!"
        return 1
    }
...
fn_wallcache() {
    ...
    if [ "$is_video" -eq 1 ]; then
        if [ ! -e "$thmbDir/$x_hash.thmb" ] || [ ! -e "$thmbDir/$x_hash.sqre" ] || [ ! -e "$thmbDir/$x_hash.blur" ] || [ ! -e "$thmbDir/$x_hash.quad" ] || [ ! -e "$dcolDir/$x_hash.dcol" ]; then
            ...
            notify-send -a "shade wallpaper" "Extracting thumbnail from video wallpaper..."
...
fn_envar_cache() {
    if command -v rofi &>/dev/null; then
        if [[ ! $XDG_DATA_DIRS =~ share/shade ]]; then
            mkdir -p "$XDG_DATA_HOME/rofi/themes"
            ln -snf "$XDG_DATA_HOME/shade/rofi/themes"/* "$XDG_DATA_HOME/rofi/themes/"
        fi
    fi
}
...
wallpaper_cache_commence() {
    ...
    while getopts "w:t:f" option; do
        ...
        w)
            ...
            ;;
        t)
            cacheIn="$(dirname "$SHADE_THEME_DIR")/$OPTARG"
            if [ ! -d "$cacheIn" ]; then
            ...
            ;;
        f)
            cacheIn="$(dirname "$SHADE_THEME_DIR")"
            mode="_force"
            ;;
        *)
            echo "... invalid option ..."
