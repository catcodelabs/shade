#!/usr/bin/env python3
"""
shade's system Lua environment manager using system luarocks
usage: lua_env.py [command] <options>


Commands:
    create     Ensure system luarocks is available and install bootstrap packages
    destroy    Remove the shade Lua rocks tree
    rebuild    Recreate the shade Lua rocks tree and reinstall packages
...
SHADE_STATE_DIR = os.path.join(XDG_STATE_HOME, "shade")
LUA_ENV_DIR = os.path.join(SHADE_STATE_DIR, "lua_env")
...
ROCKS_SNAPSHOT = os.path.join(SHADE_STATE_DIR, "luarocks_env.json")
...
def install_bootstrap(bootstrap_install, force=False):
    ...
    if skipped:
        ...
        print("[lua_env] install their build dependencies and run 'shade-shell luainit' to retry")
...
def main():
    parser = argparse.ArgumentParser(
        prog="lua_env",
        usage="lua_env.py [command] <options>",
        description="shade's system Lua environment manager",
    ...
    subparsers.add_parser("destroy", help="Remove the shade Lua rocks tree")
    subparsers.add_parser("rebuild", help="Recreate the shade Lua rocks tree and reinstall packages")
