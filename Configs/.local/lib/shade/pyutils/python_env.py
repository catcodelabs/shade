...
def get_venv_path() -> str:
    ...
    return os.path.join(str(xdg_base_dirs.xdg_state_home()), "shade", "python_env")
...
def check_uv_version(uv_path: str) -> None:
    ...
    if version < MIN_UV_VERSION:
        ...
        raise RuntimeError(
            ...
            f"shade requires uv >= {min_version_str} for sync/add/remove. "
...
def ensure_venv(venv_path: str) -> None:
    ...
    if result.returncode != 0:
        ...
        notify.send("shade UV", f"Failed to create virtual environment:\n{err}", urgency="critical")
        raise RuntimeError(f"Failed to create virtual environment: {err}")



def get_uv() -> str:
    """Finds or installs the 'uv' executable in the shade venv."""
    venv_path = get_venv_path()
    uv_venv = os.path.join(venv_path, "bin", "uv")

    # Check if uv exists in the shade venv
...
def setup_venv_with_uv() -> None:
    ...
    notify.send("shade UV", "✅ Python environment and uv installed")
...
def run_uv(
    ...
    try:
        ...
        notify.send("shade UV", str(e), urgency="critical")
        raise
    ...
    if stream:
        ...
        if result.returncode != 0:
            notify.send("shade UV", "Command failed", urgency="critical")
...
def create_venv() -> None:
    ...
    if os.path.exists(venv_path):
        if is_venv_valid(venv_path):
            notify.send("shade UV", "ℹ️ Virtual environment already exists and is valid")
            return
        notify.send("shade UV", "⚠️ Broken venv detected, recreating…")
        destroy_venv(venv_path)
    run_uv(["sync"], notify_msg="⏳ Creating virtual environment...")
    notify.send("shade UV", "✅ Virtual environment ready")
...
def destroy_venv(venv_path=None) -> None:
    ...
    if os.path.exists(venv_path):
        ...
        notify.send("shade UV", "🗑️ Virtual environment removed")
...
def rebuild_venv() -> None:
    ...
    run_uv(["sync"])
    notify.send("shade UV", "✅ Rebuild complete")
...
def sync_packages() -> None:
    ...
    notify.send("shade UV", "✅ Dependencies are up to date", replace_id=9)
...
def install_package(package: str | Iterable[str]) -> None:
    ...
    if not pkgs:
        notify.send("shade UV", "No packages specified for installation", urgency="warning")
        return

    notify.send("shade UV", f"Installing {', '.join(pkgs)}...")
    try:
        run_uv(["add"] + pkgs, stream=True)
    except RuntimeError as e:
        notify.send("shade UV", f"Error installing packages: {e}", urgency="critical")
        raise
...
def uninstall_package(package: str | Iterable[str]) -> None:
    ...
    if not pkgs:
        notify.send("shade UV", "No packages specified for uninstallation", urgency="warning")
        return

    notify.send("shade UV", f"Uninstalling {', '.join(pkgs)}...")
    try:
        run_uv(["remove"] + pkgs, stream=True)
    except RuntimeError as e:
        notify.send("shade UV", f"Error uninstalling packages: {e}", urgency="critical")
        raise
...
def v_import(module_name: str, auto_install: bool = True, extra: str = None) -> ModuleType:
    ...
    try:
        ...
        notify.send("shade UV", f"Installing missing module: {module_name}")
        if extra:
        ...
        try:
            ...
            notify.send("shade UV", f"{module_name} installed successfully")
            return module
...
def main(argv) -> None:
    parser = argparse.ArgumentParser(
        ...
        description="shade's Python virtual environment manager",
    ...
    uv_p = subparsers.add_parser("uv", help="Run a raw uv command. Use --shade to scope shade's venv")
    uv_p.add_argument("--shade", action="store_true", help="Run within shade's virtualenv context")
