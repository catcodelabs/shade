...
-- Notification helper for shade. Uses notify-send in the background so the
...
function M.send(summary, body, opts)
    ...
    if has_notify_send() then
        ...
        local cmd = string.format('notify-send -a "shade Power" -t %d -r %d -u %s%s %s %s >/dev/null 2>&1 &',
