...
"""
shade Shell Completions Engine — universal metadata parser.
...
# ──────────────────────────────────────────────────────────────
# Built-in command list (parsed from shade-shell annotations)
# ──────────────────────────────────────────────────────────────
SHADE_SHELL_PATH = (HERE.parent.parent / "bin" / "shade-shell").resolve()
if not SHADE_SHELL_PATH.is_file():
    SHADE_SHELL_PATH = Path("/usr/local/bin/shade-shell")
...
def _ensure_builtins():
    """Parse @cmd annotations from shade-shell and populate BUILT_IN_COMMANDS."""
    if BUILT_IN_COMMANDS:
        return
    meta = parse_new_style(str(SHADE_SHELL_PATH))
    cmds = {}
    for cmd_name, info in meta.get("commands", {}).items():
        desc = info.get("desc", "").strip()
        cmds[cmd_name] = desc or "shade command"
...
def get_script_paths() -> list:
    ...
    Checks SHADE_SCRIPTS_PATH env var first, then falls back to
    scanning the shade lib directory for scripts with metadata.
    """
    # Mirror shade-shell defaults when SHADE_SCRIPTS_PATH is missing so
    # completions still include user/config scripts in non-initialized shells.
    xdg_config_home = os.environ.get("XDG_CONFIG_HOME", os.path.expanduser("~/.config"))
    xdg_data_home = os.environ.get("XDG_DATA_HOME", os.path.expanduser("~/.local/share"))
    default_paths = [
        os.path.join(xdg_config_home, "shade", "scripts"),
        str(HERE),
        os.path.join(xdg_data_home, "waybar", "scripts"),
        os.path.join(xdg_config_home, "waybar", "scripts"),
    ]
    env_paths = os.environ.get("SHADE_SCRIPTS_PATH", "")
    search_paths = [p for p in env_paths.split(":") if p.strip()] if env_paths else default_paths[:]
    seen = set()
    paths = []

    # Always include the shade lib dir as a fallback path.
...
def get_all_entries() -> list:
    ...
    for name in BUILT_IN_COMMANDS:
        entries[name] = BUILT_IN_DESCRIPTIONS.get(name, "shade command")
...
def output_completion_file(shell: str):
    ...
    template = COMPLETIONS_DIR / f"shade-shell.{shell}"
    if template.is_file():
...
def main():
    ...
    if cmd == "--list-builtins":
        ...
        _print_desc_items([(name, BUILT_IN_DESCRIPTIONS.get(name, "shade command")) for name in BUILT_IN_COMMANDS])
