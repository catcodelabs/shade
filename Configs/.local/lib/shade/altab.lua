...
local parser = argparse("shade-shell altab", "Window Alt-Tab Switcher (index-based, optimized)")
...
parser:flag("--log", "Enable file logging to XDG_RUNTIME_DIR/shade-altab/log.txt")
...
if DEBUG then
    logger.level = "debug"
    logger.debug("[shade-altab] debug mode enabled")
end
...
local state_dir = xdg_runtime .. "/shade-altab"
...
if DEBUG and LOG then
    append_log_file("DEBUG", "[shade-altab] debug mode enabled")
end

local function log(msg)
    local formatted = string.format("[shade-altab] %s", tostring(msg))
