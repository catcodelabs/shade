...
rofiStyleDir="$SHARE_DIR/shade/rofi/themes"
rofiAssetDir="$SHARE_DIR/shade/rofi/assets"
...
if [ -n "$RofiSel" ]; then
    ...
    notify-send -a "shade Alert" -r 2 -t 2200 -i "$rofiAssetDir/$RofiSel.png" " style $RofiSel applied..."
fi
if [ -n "$ROFI_LAUNCH_STYLE" ]; then
    notify-send -a "shade Alert" -r 3 -u critical "Style: '$ROFI_LAUNCH_STYLE' is explicitly set, remove it in ~/.config/shade/config.toml for changes to take effect."
fi
