...
[[ -f "${LIB_DIR}/shade/shutils/l10n.sh" ]] && source "${LIB_DIR}/shade/shutils/l10n.sh"
...
temp_screenshot=${XDG_RUNTIME_DIR:-/tmp}/shade_screenshot.png
...
take_screenshot() {
    ...
    command=("$LIB_DIR/shade/screenshot/grimblast" "${extra_args[@]}" "copysave" "${mode}" "${target_file}")
    ...
    if eval "${command[*]}"; then
        ...
        if ! run_annotation_tool; then
            send_notifs -r 9 -a "shade Alert" "Screenshot Error" "Failed to open annotation tool"
            return 1
        fi
    else
        send_notifs -a "shade Alert" "Screenshot Error" "Failed to take screenshot"
        return 1
    fi
}
ocr_screenshot() {
    ...
    shift
    local extra_args=("$@")
    if "$LIB_DIR/shade/screenshot/grimblast" "${extra_args[@]}" copysave "$mode" "$temp_screenshot"; then
        source "${LIB_DIR}/shade/shutils/ocr.sh"
        source ${XDG_STATE_HOME}/shade/config
        ...
        if ! ocr_extract "$temp_screenshot"; then
            send_notifs -r 9 -a "shade Alert" "OCR: extraction error" -e -i "dialog-error"
            return 1
        fi
    else
        send_notifs -a "shade Alert" "OCR: screenshot error" -e -i "dialog-error"
        return 1
    fi
    exit 0
}
qr_screenshot() {
    ...
    local extra_args=("$@")
    if "$LIB_DIR/shade/screenshot/grimblast" "${extra_args[@]}" copysave "$mode" "$temp_screenshot"; then
        source "${LIB_DIR}/shade/shutils/qr.sh"
        ...
        if ! qr_extract "$temp_screenshot"; then
            send_notifs -r 9 -a "shade Alert" "QR: extraction error" -e -i "dialog-error"
            return 1
        fi
    else
        send_notifs -a "shade Alert" "QR: screenshot error" -e -i "dialog-error"
        return 1
    fi
}
...
if [ -f "$save_dir/$save_file" ] && [[ "${SCREENSHOT_NOTIFY}" != false ]]; then
    send_notifs -r 9 -a "shade Alert" -i "$save_dir/$save_file" "saved in $save_dir"
    exit 0
fi
