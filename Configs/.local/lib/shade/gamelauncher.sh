...
if [ "$#" -lt 2 ] && [[ $1 =~ ^[0-9]+$ ]]; then
    ...
    argparse_program "shade-shell gamelauncher"
    argparse_header "shade Game Launcher"
...
case ${style:-5} in
    ...
    "monitor_info=(\(.width) \(.height) \(.scale) \(.x) \(.y)) reserved_info=(\(.reserved | join(" ")))"')"
        ...
        BG=$HOME/.local/share/shade/rofi/assets/steamdeck_holographic.png
        BGfx=$HOME/.cache/shade/landing/steamdeck_holographic_${monitor_width}x$monitor_height.png
        if [ ! -e "$BGfx" ]; then
...
case "$backend" in
    steam)
        backend_command=("${XDG_STATE_HOME:-$HOME/.local/state}/shade/python_env/bin/python" "$LIB_DIR/shade/gamelauncher/steam.py" --rofi-string)
        ;;
    lutris)
        backend_command=("${XDG_STATE_HOME:-$HOME/.local/state}/shade/python_env/bin/python" "$LIB_DIR/shade/gamelauncher/lutris.py" --rofi-string)
        ;;
    *)
        backend_command=("${XDG_STATE_HOME:-$HOME/.local/state}/shade/python_env/bin/python" "$LIB_DIR/shade/gamelauncher/catalog.py" --rofi-string)
