...
export SHADE_CONFIG_HOME="$XDG_CONFIG_HOME/shade"
export SHADE_DATA_HOME="$XDG_DATA_HOME/shade"
export SHADE_CACHE_HOME="$XDG_CACHE_HOME/shade"
export SHADE_STATE_HOME="$XDG_STATE_HOME/shade"
export SHADE_RUNTIME_DIR="$XDG_RUNTIME_DIR/shade"
export ICONS_DIR="$XDG_DATA_HOME/icons"
export FONTS_DIR="$XDG_DATA_HOME/fonts"
export THEMES_DIR="$XDG_DATA_HOME/themes"
export scrDir="${LIB_DIR:-$HOME/.local/lib}/shade"
export confDir="${XDG_CONFIG_HOME:-$HOME/.config}"
export shadeConfDir="$SHADE_CONFIG_HOME"
export cacheDir="$SHADE_CACHE_HOME"
export thmbDir="$SHADE_CACHE_HOME/thumbs"
export dcolDir="$SHADE_CACHE_HOME/dcols"
export iconsDir="$ICONS_DIR"
export themesDir="$THEMES_DIR"
export fontsDir="$FONTS_DIR"
export hashMech="sha1sum"

export SHADE_STATUS_CACHE_FAILED=3
export SHADE_STATUS_COLOURS_FAILED=4

##
# Creates the directories shade writes its own generated state into. A template
...
#
# Globals:
#   SHADE_STATE_HOME, SHADE_CACHE_HOME, SHADE_RUNTIME_DIR, XDG_CONFIG_HOME
##
shade_state_dirs() {
    local dir
    for dir in "$SHADE_STATE_HOME/lua_state" "$SHADE_CACHE_HOME/wallbash" "$SHADE_RUNTIME_DIR" \
        "${XDG_CONFIG_HOME:-$HOME/.config}/hypr/themes"; do
        [ -d "$dir" ] && continue
        if ! mkdir -p "$dir"; then
            printf '[shade] could not create %s\n' "$dir" >&2
            return 1
        fi
    done
}
shade_state_dirs
shade_state_dirs_status=$?
...
get_hashmap() {
    ...
    if [ -z "${#wallList[@]}" ] || [[ ${#wallList[@]} -eq 0 ]]; then
        if [[ $skipStrays -eq 1 ]]; then
            ...
            [ -n "$no_notify" ] && notify-send -a "shade Alert" "WARNING: No compatible wallpapers found in: ${no_wallpapers[*]}"
            exit 1
        fi
    fi
...
get_themes() {
    ...
    unset thmList
    unset thmWall
    if [ ! -d "$SHADE_CONFIG_HOME/themes" ]; then
        print_log -sec "theme" -warn "themes" "no theme directory at $SHADE_CONFIG_HOME/themes"
        return 1
    fi
    while read -r thmDir; do
        ...
        thmListS+=("${thmDir##*/}")
    done < <(find -H "$SHADE_CONFIG_HOME/themes" -mindepth 1 -maxdepth 1 -type d)
...
# `shade-shell init`, which calls this) would silently die right here,
...
##
export_shade_config() {
    local user_conf_state="$XDG_STATE_HOME/shade/staterc"
    local user_conf="$XDG_STATE_HOME/shade/config"
...
if [ -z "$SHADE_THEME" ] || [ ! -d "$SHADE_CONFIG_HOME/themes/$SHADE_THEME" ]; then
    get_themes
    SHADE_THEME="${thmList[0]}"
fi
SHADE_THEME_DIR="$SHADE_CONFIG_HOME/themes/$SHADE_THEME"
WALLBASH_DIRS=(
    "$XDG_CONFIG_HOME/wallbash"
    "$XDG_CONFIG_HOME/shade/wallbash"
    "$XDG_DATA_HOME/wallbash"
    "$XDG_DATA_HOME/shade/wallbash"
    "/usr/local/share/shade/wallbash"
    "/usr/share/shade/wallbash")
wallbashDirs=("${WALLBASH_DIRS[@]}")
export SHADE_THEME SHADE_THEME_DIR WALLBASH_DIRS wallbashDirs enableWallDcol
if [ -n "$HYPRLAND_INSTANCE_SIGNATURE" ]; then
    hypr_border="$(hyprctl -j getoption decoration:rounding | jq '.int')"
    hypr_width="$(hyprctl -j getoption general:border_size | jq '.int')"
fi
export hypr_border=${hypr_border:-${SHADE_BORDER_RADIUS:-2}}
export hypr_width=${hypr_width:-${SHADE_BORDER_WIDTH:-2}}
pkg_installed() {
    local pkgIn=$1
    if command -v "$pkgIn" &>/dev/null; then
        ...
        return 0
    elif shade-shell pm.sh pq "$pkgIn" &>/dev/null; then
        return 0
    else
        return 1
    fi
...
set_conf() {
    local varName="$1"
    local varData="$2"
    touch "$XDG_STATE_HOME/shade/staterc"
    if [ "$(grep -c "^$varName=" "$XDG_STATE_HOME/shade/staterc")" -eq 1 ]; then
        sed -i "/^$varName=/c$varName=\"$varData\"" "$XDG_STATE_HOME/shade/staterc"
    else
        echo "$varName=\"$varData\"" >>"$XDG_STATE_HOME/shade/staterc"
    fi
}
set_hash() {
    local hashImage="$1"
...
check_package() {
    local lock_file="${XDG_RUNTIME_DIR:-/tmp}/shade/__package.lock"
    mkdir -p "${XDG_RUNTIME_DIR:-/tmp}/shade"
    if [ -f "$lock_file" ]; then
        return 0
    fi
    for pkg in "$@"; do
...
get_hyprConf() {
    local hyArg="$1"
    local file="${2:-"$SHADE_THEME_DIR/hypr.theme"}"
    local hyVar="$hyArg"
    local hyType=""
    ...
    if [ -z "$gsVal" ] || [[ $gsVal == \$* ]]; then
        ...
        *) grep "^[[:space:]]*\$default.$hyVar\s*=" \
            "$XDG_DATA_HOME/shade/shade.conf" \
            "$XDG_DATA_HOME/shade/hyprland.conf" \
            "/usr/local/share/shade/shade.conf" \
            "/usr/local/share/shade/hyprland.conf" \
            "/usr/share/shade/shade.conf" \
            "/usr/share/shade/hyprland.conf" 2>/dev/null | cut -d '=' -f2 | sed 's/^[[:space:]]*//;s/[[:space:]]*$//' | head -n 1 ;;
        esac
    else
        echo "$gsVal"
    fi
...
paste_string() {
    ...
    ignore_paste_file="$SHADE_STATE_HOME/ignore.paste"
...
wallbash_state_is_complete() {
    local required=(
        ...
        "$SHADE_STATE_HOME/lua_state/colors.lua"
        "$SHADE_STATE_HOME/lua_state/ui.lua"
    )
    local artefact
...
accepted_mime_types() {
    ...
    for mime_type in "${mime_types_array[@]}"; do
        if file --mime-type -b "$file" | grep -q "^$mime_type"; then
            ...
            notify-send -u critical -a "shade-Alert" "File type not supported for this wallpaper backend."
        fi
    done
}
dconf_write() {
...
export -f get_hyprConf get_monitor_scale get_rofi_pos is_hovered toml_write get_hashmap get_aurhlpr set_conf set_hash check_package get_themes print_log pkg_installed paste_string extract_thumbnail accepted_mime_types dconf_write send_notifs export_shade_config wallbash_state_is_complete
...
##
if [ "${shade_state_dirs_status:-0}" -ne 0 ]; then
    return "$shade_state_dirs_status" 2>/dev/null || exit "$shade_state_dirs_status"
fi
