...
def fetch_data():
    global JSON_DATA
    json_file_path = os.path.join(CLONE_DIR, "shade-themes.json")
...
def get_theme_preview(theme):
    ...
    if theme == "[CONFIRM]":
        ...
        preview_text += (
            "👋 Let's shaderate your system with more cool themes!\n\n"
            "  [TAB] to mark a theme\n"
...
def fzf_menu():
    try:
        ...
        subprocess.run(["shade-shell", "reload"], check=True)
...
def fetch_all_themes():
    ...
    themes_dir = os.path.join(config_home, "shade/themes")
...
def main():
    parser = argparse.ArgumentParser(
        description="Imports themes from shade-gallery repository",
        epilog="Env:\n"
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="THEME",
        help="Fetch and update a specific theme by name (`all` to fetch all themes located in 'xdg_config/shade/themes')",
    )
