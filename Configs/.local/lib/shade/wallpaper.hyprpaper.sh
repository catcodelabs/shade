...
selected_wall="${1:-${XDG_CACHE_HOME:-$HOME/.cache}/shade/wall.set}"
...
if [[ -n $HYPRLAND_INSTANCE_SIGNATURE ]]; then
    ...
    cat <<EOF >"$XDG_STATE_HOME/shade/hyprpaper.conf"
splash = false
...
EOF
    ...
    if systemctl --user is-active --quiet hyprpaper.service; then
        ...
        app.sh -- hyprpaper --config "$XDG_STATE_HOME/shade/hyprpaper.conf"
    fi
