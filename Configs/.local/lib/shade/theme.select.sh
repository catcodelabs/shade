...
selector_menu() {
    ...
    if [ -n "$RofiSel" ]; then
        ...
        notify-send -a "shade Alert" -i "$rofiAssetDir/theme_style_$selectedStyle.png" "Style $selectedStyle applied..."
...
if [ -n "$rofiSel" ]; then
    "$LIB_DIR/shade/theme.switch.sh" -s "$rofiSel"
    notify-send -a "shade Alert" -i "$iconsDir/Wallbash-Icon/shade.png" " $rofiSel"
fi
