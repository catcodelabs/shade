...
if [[ $SHADE_SHELL_INIT -ne 1 ]]; then
    eval "$(shade-shell init)"
else
    export_shade_config
fi

source "$LIB_DIR/shade/wallpaper/help.sh"
source "$LIB_DIR/shade/wallpaper/core.sh"
source "$LIB_DIR/shade/wallpaper/select.sh"
...
run_cache_command() {
    ...
    case "$cache_mode" in
    current)
        "$LIB_DIR/shade/wallpaper/cache.sh" commence
        ;;
    w | wall)
        ...
        "$LIB_DIR/shade/wallpaper/cache.sh" commence -w "$cache_arg"
        ;;
    t | theme)
        if [ -z "$cache_arg" ]; then
        ...
        "$LIB_DIR/shade/wallpaper/cache.sh" commence -t "$cache_arg"
        ;;
    f | full | all)
        "$LIB_DIR/shade/wallpaper/cache.sh" commence -f
        ;;
    *)
...
setup_wallpaper_targets() {
    ...
    if [ "$set_as_global" == "true" ]; then
        wallSet="$SHADE_THEME_DIR/wall.set"
        wallCur="$SHADE_CACHE_HOME/wall.set"
        wallSqr="$SHADE_CACHE_HOME/wall.sqre"
        wallTmb="$SHADE_CACHE_HOME/wall.thmb"
        wallBlr="$SHADE_CACHE_HOME/wall.blur"
        wallQad="$SHADE_CACHE_HOME/wall.quad"
        wallDcl="$SHADE_CACHE_HOME/wall.dcol"
    elif [ -n "$wallpaper_backend" ]; then
        mkdir -p "$SHADE_CACHE_HOME/wallpapers"
        wallCur="$SHADE_CACHE_HOME/wallpapers/$wallpaper_backend.png"
        wallSet="$SHADE_THEME_DIR/wall.$wallpaper_backend.png"
    else
        wallSet="$SHADE_THEME_DIR/wall.set"
    fi
    if [ ! -e "$wallSet" ]; then
...
wallpaper_backend_status() {
    case "$1" in
    "${SHADE_STATUS_CACHE_FAILED:-3}" | "${SHADE_STATUS_COLOURS_FAILED:-4}") printf '%s\n' 1 ;;
    *) printf '%s\n' "$1" ;;
    esac
}
...
main() {
    ...
    if [ -n "$wallpaper_setter_flag" ]; then
        ...
        select)
            ...
            if [ "$output_flag" != "true" ]; then
                local _cache_pid
                "$LIB_DIR/shade/wallpaper/cache.sh" commence &>/dev/null &
                _cache_pid=$!
    ...
    if [ -f "$LIB_DIR/shade/wallpaper.$wallpaper_backend.sh" ] && [ -n "$wallpaper_backend" ]; then
        print_log -sec "wallpaper" "Using backend: $wallpaper_backend"
        "$LIB_DIR/shade/wallpaper.$wallpaper_backend.sh" "$wallSet" || backend_status=$?
    else
        if command -v "wallpaper.$wallpaper_backend.sh" >/dev/null; then
            ...
            print_log -warn "wallpaper" "Created: $SHADE_CACHE_HOME/wallpapers/$wallpaper_backend.png instead"
        fi
    fi
    if [ "$wallpaper_setter_flag" == "select" ]; then
        if [ -e "$(readlink -f "$wallSet")" ]; then
            if [ "$set_as_global" == "true" ]; then
                notify-send -a "shade Alert" -i "$selected_thumbnail" "$selected_wallpaper"
            else
                notify-send -a "shade Alert" -i "$selected_thumbnail" "$selected_wallpaper set for $wallpaper_backend"
            fi
        else
            notify-send -a "shade Alert" "Wallpaper not found"
        fi
    fi
