...
if set -q SHADE_ACTIVATED
    exit 0
end

set -x SHADE_ACTIVATED 1
set -x SHADE_MODE ""
...
function setup_xdg
    ...
    set -gx XDG_RUNTIME_DIR (test -n "$XDG_RUNTIME_DIR"; and echo $XDG_RUNTIME_DIR; or shade_runtime_dir)
end
...
function shade_find_lua
    shade_find_bin $LUA lua lua5.5 lua5.4 lua5.3
end

function shade_find_luarocks
    shade_find_bin $LUAROCKS luarocks luarocks-5.5 luarocks-5.4 luarocks-5.3
end

function check_lua_runtime
    set -l missing
    shade_find_lua >/dev/null; or set -a missing lua
    shade_find_luarocks >/dev/null; or set -a missing luarocks
...
function setup_lua_mode
    ...
    set -gx SHADE_MODE "lua"
    set -gx SHADE_FEATURE_LUA 1
end

function setup_session
    set -gx XDG_CURRENT_DESKTOP (test -n "$XDG_CURRENT_DESKTOP"; and echo $XDG_CURRENT_DESKTOP; or echo "shade")
    set -gx XDG_SESSION_DESKTOP (test -n "$XDG_SESSION_DESKTOP"; and echo $XDG_SESSION_DESKTOP; or echo "shade")
    set -gx XDG_SESSION_TYPE (test -n "$XDG_SESSION_TYPE"; and echo $XDG_SESSION_TYPE; or echo "wayland")
end

function shade_activate
    setup_xdg
    setup_session
    if setup_lua_mode
        :
