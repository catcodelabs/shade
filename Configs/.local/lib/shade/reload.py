#!/usr/bin/env python3
"""Reload script for shade components."""

# @name: reload
# @ver: 0.1.0
# @short: reload
# @cmd: all
# @cmd.desc: Reload all shade components
# @cmd: python
...
def reload_lua() -> bool:
    ...
    lua_script = LIB_DIR / "shade" / "pyutils" / "lua_env.py"
    if not lua_script.exists():
        print_log("-err", f"Lua env script not found at {lua_script}")
        return False

    print_log("-sec", "shade", "Reloading Lua environment...")
...
def main():
    parser = argparse.ArgumentParser(description="Reload shade components.")
    parser.add_argument(
        "targets",
        nargs="*",
        default=["all"],
    ...
    print_log("-sec", "shade", f"Reloading shade components: {', '.join(targets_to_run)}")
