...
shade.get_logical_monitor = function()
    ...
    local cfg_m = shade.config.monitor and shade.config.monitor.edge_margin or {0}
...
shade.handle.float_size_bounds = function(win)
    if not win or not win.floating then
        return
    end
    local cfg = shade.config.window and shade.config.window.float_size_bounds
    if not cfg or not cfg.enabled then
        return
    end
    local no_bounds = shade.config.window.float.no_bounds or {}
    ...
    local l_mon = shade.get_logical_monitor()
    if not l_mon then
        return
    end
...
shade.handle.float_follow_cursor = function(win)
    if not win or not win.floating then
        return
    end
    local cfg = shade.config.window and shade.config.window.float_follow_cursor
    if not cfg or not cfg.enabled then
        return
    end
...
shade.handle.follow_cursor_quadrant = function(win, opts)
    if not win or not win.floating then
        return
    end
