...
-- shade.binds._active. It deduplicates bindings only when
-- shade.binds.dedup is enabled, and only for bindings with the same
...
local function has_dedup_field(opts)
    ...
    local fields = shade.binds.dedup_fields
...
shade = shade or {}
shade.binds = shade.binds or {}

shade.binds.dedup = shade.binds.dedup == nil and false or shade.binds.dedup
shade.binds.dedup_fields =
    shade.binds.dedup_fields or
    {
        "locked",
        "release",
        "click",
...
shade.binds.normalize = normalize
shade.binds.canonicalize = canonicalize
...
shade.binds._active = shade.binds._active or {}

local orig_add = hl.bind

hl.bind = function(keycombo, action, ...)
    local normalized = shade.binds.normalize(keycombo)
    local opts = find_options(...)
    local signature = serialize_flags(opts)
    local dedup_id = canonicalize(keycombo) .. "|" .. signature

    if normalized ~= "" and shade.binds.dedup then
        local registered = shade.binds._active[dedup_id]
        if registered then
    ...
    if normalized ~= "" then
        shade.binds._active[dedup_id] = normalized
        keycombo = normalized
    end
