--- @module shade.path
--- Resolves the XDG and shade directories the Hyprland Lua configuration reads
--- from, and exposes them globally as `shade.path`.
---
...
---
--- Usage:
---   local config_file = shade.path.config and (shade.path.config .. "/shade/config.toml")
...
--- @field lib string|nil Directory holding the shade Lua and shell libraries.
P.lib = first_directory(HOME and HOME .. "/.local/lib", {"/usr/local/lib", "/usr/lib"})

--- @field share string|nil Directory holding the shipped shade data files.
