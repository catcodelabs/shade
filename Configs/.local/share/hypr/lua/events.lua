local floating_window_boundary = function(win)
	-- shade's helper to limit the window size when floating, based on the monitor's usable area.
	-- This prevents windows from opening larger than the screen, eg annoying  dolphin window
	if not win or not win.floating then
		return
	end

	if shade.handle and shade.handle.float_size_bounds then
		shade.handle.float_size_bounds(win)
	end
end

-- spawns floating windows
local floating_window_follow_cursor = function(win)
	if shade.handle and shade.handle.float_follow_cursor then
		shade.handle.float_follow_cursor(win)
	end
end
