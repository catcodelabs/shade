---
shade exposes `xdg_config/shade/config.toml` file for users to modify. This lets users have the ability to interact the scripts without using command arguments.
...
| Key | Description | Default |
| --- | ----------- | ------- |
| browser | Browser launcher command. | shade-shell open --fall firefox web-browser |
| editor | Editor launcher command. | shade-shell open --fall code-oss code-editor |
| explorer | File manager launcher command. | shade-shell open --fall dolphin file-manager |
| lockscreen | Lockscreen command. | shade-shell lock-session |
| quickapps | Command used for quickapps. | kitty |
| terminal | Terminal launcher command. | shade-shell app -T |

### [desktop.ui]

Desktop UI settings for shade and cross-compositor support.

| Key | Description | Default |
| --- | ----------- | ------- |
| bar_font | Bar font used by shade. | JetBrainsMono Nerd Font |
| button_layout | Button layout string for UI controls. |  |
| code_theme | Code theme used by shade. |  |
| color_scheme | Color scheme for shade UI. | prefer-dark |
| cursor_size | Cursor size used by shade. | 24 |
| cursor_theme | Cursor theme used by shade. | Bibata-Modern-Ice |
| document_font | Document font used by shade UI. | Cantarell |
| document_font_size | Document font size for shade UI. | 10 |
| font | Default font for shade UI. | Cantarell |
| font_antialiasing | Font antialiasing mode for shade UI. | rgba |
| font_hinting | Font hinting mode for shade UI. |  |
| font_size | Font size for shade UI. | 10 |
| gtk_theme | GTK theme for shade UI elements. | Wallbash-Gtk |
| shade_theme | shade theme name. | shade |
| icon_theme | Icon theme for shade UI. | Tela-circle-dracula |
| menu_font | Menu font used by shade. | JetBrainsMono Nerd Font |
| monospace_font | Monospace font for shade UI. | CaskaydiaCove Nerd Font Mono |
| monospace_font_size | Monospace font size for shade UI. | 9 |
| notification_font | Notification font used by shade. | Mononoki Nerd Font Mono |
| sddm_theme | sddm theme used by shade. |  |
...
| Key | Description | Default |
| --- | ----------- | ------- |
| applet_bluetooth | Bluetooth applet. | shade-shell app -u shade-$XDG_SESSION_DESKTOP-bluetooth-applet.service -t service -- blueman-applet |
| applet_network_manager | Network manager applet. | shade-shell app -u shade-$XDG_SESSION_DESKTOP-network-manager-applet.service -t service -- nm-applet --indicator |
| applet_removable_media | Removable media applet. | shade-shell app -u shade-$XDG_SESSION_DESKTOP-removable-media-applet.service -t service -- udiskie --no-automount --smart-tray |
| auth_dialogue | Authentication dialogue. | shade-shell app -t service -- polkitkdeauth.sh |
| bar | Bar. | shade-shell app -u shade-$XDG_SESSION_DESKTOP-bar.scope -t scope -- waybar.py --watch |
| battery_notify | Battery notification script. | shade-shell app -u shade-$XDG_SESSION_DESKTOP-battery-notify.service -t service -- batterynotify.lua |
| blue_light_filter_daemon | Blue-light filter daemon. | shade-shell app -u shade-$XDG_SESSION_DESKTOP-blue-light-filter.service -t service -- hyprsunset |
| clipboard_persist | Clipboard persist daemon. | shade-shell app -u shade-$XDG_SESSION_DESKTOP-clipboard-persist.service -t service wl-clip-persist --clipboard regular |
...
| idle_daemon | Idle daemon. | shade-shell app -u shade-$XDG_SESSION_DESKTOP-idle.service -t service -- hypridle |
| image_clipboard | Image clipboard daemon. | shade-shell app -u shade-$XDG_SESSION_DESKTOP-image-clipboard.service -t service wl-paste --type image --watch cliphist store |
| notifications | Notification daemon. | shade-shell app -u shade-$XDG_SESSION_DESKTOP-notifications.service -t service -- dunst |
...
| text_clipboard | Text clipboard daemon. | shade-shell app -u shade-$XDG_SESSION_DESKTOP-text-clipboard.service -t service wl-paste --type text --watch cliphist store |
| wallpaper | Wallpaper script. | shade-shell app -u shade-$XDG_SESSION_DESKTOP-wallpaper.service -t service -- wallpaper.sh --start --global |
| xdg_portal_reset | XDG portal reset script. | shade-shell resetxdgportal.sh |
...
shade-shell rofi.bookmarks.sh configuration.
...
'shade-shell hyprlock select' configuration.
...
shade-shell rofi.websearch.sh configuration.
