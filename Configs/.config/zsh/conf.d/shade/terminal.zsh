...
function _dedup_zsh_plugins {
    ...
    # Load Plugins
    shade_plugins=(git zsh-256color zsh-autosuggestions zsh-syntax-highlighting)
    plugins+=("${plugins[@]}" "${shade_plugins[@]}")
    # Deduplicate plugins
    plugins=("${plugins[@]}")
    plugins=($(printf "%s\n" "${plugins[@]}" | sort -u))
...
function _load_deferred_plugin_system_by_shade() {

    # Exit early if SHADE_ZSH_DEFER is not set to 1
    if [[ "${SHADE_ZSH_DEFER}" != "1" ]]; then
        unset -f _load_deferred_plugin_system_by_shade
        return
    fi
...
function _load_compinit() {
    ...
    if [[ -n ${ZDOTDIR}/.zcompdump(#qN.mh+${SHADE_ZSH_COMPINIT_CHECK:-1}) ]]; then
        compinit
    else
        compinit -C
    fi
...
function _load_prompt() {
    ...
    if ! source ${ZDOTDIR}/prompt.zsh >/dev/null 2>&1; then
        [[ -f $ZDOTDIR/conf.d/shade/prompt.zsh ]] && source $ZDOTDIR/conf.d/shade/prompt.zsh
    fi
}
...
SHADE_ZSH_DEFER="1"      #Unset this variable in $ZDOTDIR/user.zsh to disable shade's deferred Zsh loading.
SHADE_ZSH_PROMPT="1"     #Unset this variable in $ZDOTDIR/user.zsh to disable shade's prompt customization.
SHADE_ZSH_NO_PLUGINS="0" #Set this variable to "1" in $ZDOTDIR/user.zsh to disable shade's Zsh plugin loading.
...
PM_COMMAND=(shade-shell pm)

# Optionally load user configuration // useful for customizing the shell without modifying the main file
if [[ -f $HOME/.shade.zshrc ]]; then
    source $HOME/.shade.zshrc # for backward compatibility
...
if [[ ${SHADE_ZSH_NO_PLUGINS} != "1" ]]; then
    _dedup_zsh_plugins
    if [[ "$SHADE_ZSH_OMZ_DEFER" == "1" ]] && [[ -r $ZSH/oh-my-zsh.sh ]]; then
        # Loads the buggy deferred oh-my-zsh plugin system by shade // This is only for oh-my-zsh and compatibility
        _load_deferred_plugin_system_by_shade
