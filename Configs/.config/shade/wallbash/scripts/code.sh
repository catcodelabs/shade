...
# shellcheck source=$HOME/.local/bin/shade-shell
# shellcheck disable=SC1091
if ! source "$(which shade-shell)"; then
    echo "[wallbash] code :: Error: shade-shell not found."
    echo "[wallbash] code :: Is shade installed?"
    exit 1
fi

cacheDir="${cacheDir:-$XDG_CACHE_HOME/shade}"
...
for i in "${!codeConf[@]}"; do
    ...
    extTheme="$(
        "$LIB_DIR/shade/parse.json.py" -CRQ '.["workbench.colorTheme"]' "${codeConf[i]}/User/settings.json"
    )"
    if [ "${extTheme}" != "${set_theme}" ]; then
        "$LIB_DIR/shade/parse.json.py" -CU '.["workbench.colorTheme"]' "${set_theme}" "${codeConf[i]}/User/settings.json"
