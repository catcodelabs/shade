...
The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/), and this project adheres to _Modified_ [CalVer](https://calver.org/). See [Versioning](https://github.com/shade-Project/shade/blob/master/RELEASE_POLICY.md#versioning-yymq) For more info
 -->
...
- Docs: fix broken shade wiki links across the main and translated `README` files
- Waybar: correct spacing and missing icons in window module 
- Waybar: choosing a theme, or just a wallpaper within the current theme, from the shade menu, the theme module, the wallpaper widget or the macOS layout's menu no longer silently leaves the wallpaper and colour state unapplied; both paths write into `hypr/themes/colors.conf`, which triggers a Hyprland autoreload, whose reload hook sends `SIGUSR2` to the whole `shade-Hyprland-bar.service` cgroup — killing the in-flight `theme.select.sh`/`theme.switch.sh`/`wallpaper.sh` process tree along with it. These menu actions now launch them via `shade-shell app -t scope` so they run in their own cgroup instead of waybar's.
...
- Python environment: `uv sync` now targets the shade-managed venv at `~/.local/state/shade/python_env` instead of creating a project-local `.venv`; also forces `--link-mode copy` to avoid silent reflink failures on ext4 that left packages uninstalled
...
- Waybar: the theme picker opens again from the theme module, the shade menu and the macOS layout's menu; all three called `themeselect`, which the v26.7.4 migration renamed to `theme.select`
...
- wlogout: the menu is drawn with the shade layouts, icons and styles again instead of the upstream defaults
...
- Hyprland: a session started without `HYPRLAND_CONFIG`, from a TTY or a display manager, no longer trips emergency mode with `module 'lua.shade.path' not found`; the entry point resolves the modules shipped beside it from its own path
- Fish: `~/.local/bin` reaches `PATH` again, so `shade-shell` and everything the keybinds call resolve in a fish session; the directory was handed to `fish_add_path` joined to the rest of `PATH` by colons, which is one path that exists nowhere and is dropped without a word
...
- Core: Pyprland commands keep their arguments when `nc`, `socat`, and `ncat` are unavailable, so commands such as `shade-shell pypr toggle console` work through the CLI fallback again
...
- Waybar: the theme module and the shade menu call `theme.switch` again; `themeswitch` was removed and the calls failed outright
- Dolphin: the "Set As Wallpaper" service menu switches the theme again
- Fish: `HYPRLAND_CONFIG` points at the deployed `hypr/shade.lua` instead of a config the Lua release deleted, so a session started outside uwsm no longer comes up without a config
...
### Added
- GTK: shade will have our own dconf db called 'shade_hyprland'. That means dconf settings are isolated for shade Hyprland ONLY
- Hyprland: Added 'shade-shell layouts --select' to select hyprland layouts from default or custom layouts.
...
- Core: Some config changes in '~/.config/shade/config.toml', Please see the schema for more info.
...
- Hypr: removed 'hypr.altab.py' as it is too slow and and use 'hypr.altab.lua' instead. Test using 'shade-shell altab' to see if it works!

### Added
- Core: Use 'rsync' for file copy operations. Only recommended if users already have 'rsync' installed.
- Package Manager: Added --no-confirm flag to 'shade-shell pm.py' commands.
- Lua: Added 'shade-shell luainit' to initialize the Lua runtime. It is slow and should be optional for now.
- Python: 'shade-shell pyinit' will now sync like pip to preserve user packages.
...
- Hyprland: Remove 'shade.conf' as it is too brittle. Use 'hyprland.conf' instead!
...
- Updated the shade-config binary
- Python-env: Deprecate pip and use uv instead.
- Hypr: Removed '~/.config/hypr/shade.conf' as it is not being used or a long time now. Please use the 'config.toml' instead
...
### Added
- Window: added 'shade-shell window.mute' to toggle window audio. This is a python implementation for 'wl-togglesink'
...
- Session: *Experimental* feature which might be helpful as session restore in wayland is being worked on. 'shade-shell session --help' to learn more!
...
### Fixed
- Weather: Fix [#1664](https://github.com/shade-Project/shade/issues/1664)
...
- CLI: Added 'shade-shell open' for apps with mimetypes
- Core: Clean up 'shade-shell wallpaper' script
...
- Game launcher: "shade-shell game launcher" now has --style and --backend args
- Python: added pyproject.toml for ruff formatter
- Shell: Added ".editorconfig" for shell scripts.
- Cliphist: Added OCR backend. Invoking "shade-shell cliphist -scan-image" or 'Alt+V' on clipboard will extract the text of the latest image that exist in cliphist.
- Screenshot: Added QR code reading feature using 'zbar' package. No default hotkey is provided. Call it via 'shade-shell screenshot sq'
...
- Core: Moved core "color" switch inside directory in lib path. Prepare to make '~/.local/lib/shade' external only scripts and corresponding directories will be sourced or executed internally.
- Wallbash: Remove wallbash.qt as it is a simple cp command now in the qtct.dcol template
- UWSM: Start processes launched with app2unit.sh,rofi,'shade-shell app' as services

### Migration

##### Hyprland v0.53.0

- Before updating shade, Please make sure your hyprland version is 0.53.0!
- After Updating shade please make sure to fix any syntax error in ~/.config/hypr/*
...
- See common solutions https://github.com/shade-Project/shade/discussions/1477
...
- QT6CT: Added explicit font configuration for QT6 Applications see [#1309](https://github.com/shade-Project/shade/issues/1309)
- QT5CT: Added explicit font configuration for QT5 Applications see [#1309](https://github.com/shade-Project/shade/issues/1309)
- GTK3: Added explicit font configuration for GTK3 Applications see [#1309](https://github.com/shade-Project/shade/issues/1309)
...
- Fish: 'confi.d/shade.fish' is used for shade only stuff. To override this create a separate file or use 'config.fish'
...
- OCR: 'tesseract' now supports explicit language settings via 'shade/config.toml':
    '''toml
    [screenshot.ocr]
    tesseract_languages = ["eng"]
    '''
    To use text recognition bind 'shade-shell screenshot sc' to any hotkey.
...
This release delivers a new gesture syntax for hyprland v0.51.0. This is a breaking change for users of the previous gesture syntax. Please update shade before opening an issue.
...
- shade-shell: silent pyinit command
- Binds: Use 'shade-shell logout' for cleaner session logout
...
- Dunst: Fix dunst crashing when the font cannot handle unsupported characters -- Thanks to [#1131](https://github.com/shade-Project/shade/issues/1131)
...
- shade-shell: Add 'logout' command to handle with/out uwsm
- waybar: Add lighter temperature module (Needs manual setup)
- Add credits page
- waybar: Try to force initialization on restore (redundancy) might fix [#1160](https://github.com/shade-Project/shade/issues/1160)
...
Big CHANGE in shade! We are now using 'uwsm' for session management and app2unit for application management.
...
- Hyprlock: Sourcing hyprlock/shade.conf as default theme
- Core: Improved theming script stack
- Removed 'xdg-config/hypr/shade.conf' as it is too brittle. Use hyprland.conf instead!
- Moved all core hypr stuff to '~/.local/share/hypr'

### Added

- Core: Added 'app2unit.sh' as core script. This is a wrapper for the 'app' e.g. 'shade-shell app mediaplayer.py' this runs the script as systemd scope. Using app2unit.sh as 'uwsm app' is slower.
...
- Development: Added 'Scripts/shadevm' for development. See its README.md for more info.
- Package: UWSM as dependency for shade.
...
- Lib: Clean up variables that are using SHADE*, we will try to use the XDG\_* variables instead.
- Core: Fixed some issues with the theming script stack.

## v25.7.3

We use a dedicated Python environment to keep shade clean and dependency-free. Just run your scripts with 'shade-shell'— this handles the environment for you.

Examples:
 'shade-shell mediaplayer.py'
 'shade-shell waybar'
...
- shade's python environment rebuild on installation
...
- Waybar: 'shade-shell waybar --select' now will ask for **layout and style** options.
...
- Launch Scripts using 'shade-shell' instead of '$scrPath/'
